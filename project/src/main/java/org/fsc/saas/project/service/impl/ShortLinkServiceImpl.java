...
import org.fsc.saas.project.toolkit.LinkUtil;
...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        stringRedisTemplate.opsForValue().set(String.format(GOTO_SHORT_LINK_KEY, fullShortUrl), requestParam.getOriginUrl(), LinkUtil.getLinkCacheValidDate(requestParam.getValidDate()), TimeUnit.MILLISECONDS);
    ...
    private void shortLinkStats(String fullShortUrl, String gid, ServletRequest request, ServletResponse response){
        ...
        try {
            ...
            if(StrUtil.isNotBlank(infoCode) && StrUtil.equals(infoCode, "10000")){
                ...
                String provinceReplaced = province.replace("[", "").replace("]", "");
                boolean unknownFlag = provinceReplaced.isEmpty();
                linkLocaleStatsDO = LinkLocaleStatsDO.builder()
                        .fullShortUrl(fullShortUrl)
                        .province(actualProvince = unknownFlag ? "unKnown" : provinceReplaced)
                        .city(actualCity = unknownFlag ? "unKnown" : localeResultObj.getString("city").replace("[", "").replace("]", ""))
                        .adcode(unknownFlag ? "unKnown" : localeResultObj.getString("adcode").replace("[", "").replace("]", ""))
                        .country("CN")
                        .cnt(1)
                        .gid(gid)
                        .date(new Date())
                ...
                LinkAccessLogsDO linkAccessLogsDO = LinkAccessLogsDO.builder()
                        ...
                        .os(LinkUtil.getOs((HttpServletRequest) request))
                        .gid(gid)
                        .locale(StrUtil.join("-","CN", actualProvince, actualCity))
                        .device(LinkUtil.getDevice((HttpServletRequest) request))
                        .network(LinkUtil.getNetwork((HttpServletRequest) request))
                        .fullShortUrl(fullShortUrl)
                        .build();
    ...
    private String getFavicon(String url){
        ...
        if(responseCode == HttpURLConnection.HTTP_MOVED_PERM || responseCode == HttpURLConnection.HTTP_MOVED_TEMP){
            ...
            if(redirectUrl != null){
                ...
                HttpURLConnection connection1 = (HttpURLConnection) newUrl.openConnection();
